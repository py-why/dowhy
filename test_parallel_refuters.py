...
def test_refuter_source_code():
    ...
    try:
        ...
        assert 'def _simulate_confounders_effect_once(' in aucc_content, "Missing _simulate_confounders_effect_once function"
        ...
        assert 'Parallel(n_jobs=n_jobs, verbose=verbose)' in aucc_content, "Missing Parallel call in sensitivity_simulation"
...
def test_joblib_imports():
    ...
    try:        
        ...
        with open('/tmp/dowhy-410/dowhy/causal_refuters/dummy_outcome_refuter.py') as f:
            dummy_content = f.read()
        assert 'from joblib import Parallel, delayed' in dummy_content, "Missing joblib imports in dummy_outcome_refuter"
        
        with open('/tmp/dowhy-410/dowhy/causal_refuters/add_unobserved_common_cause.py') as f:
            aucc_content = f.read()
        assert 'from joblib import Parallel, delayed' in aucc_content, "Missing joblib imports in add_unobserved_common_cause"
