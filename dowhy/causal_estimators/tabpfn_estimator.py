...
class TabPFNModelWrapper:
    ...
    def predict_multiprocess(self, features: np.ndarray, want_proba: bool) -> np.ndarray:
        ...
        n_estimators_total = self.model_kwargs.get("n_estimators", 8)
        base = n_estimators_total // num_devices
        remainder = n_estimators_total % num_devices
        n_per_device_list = [base + (1 if i < remainder else 0) for i in range(num_devices)]
        ...
        for i, device_id in enumerate(self.device_ids):
            ...
            worker_kwargs = {**self.model_kwargs, "n_estimators": n_per_device_list[i]}
            p = ctx.Process(
