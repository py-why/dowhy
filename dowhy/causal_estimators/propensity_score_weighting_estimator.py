    ...
    **References**
    ...
    def estimate_effect(
        ...
        **_,
        ...
        # Binary indicator for treated units: works for any binary encoding (e.g. {1,2}, {-1,1}, strings)
        ...
        data["ips_weight"] = d / data[self.propensity_score_column] + (1 - d) / (1 - data[self.propensity_score_column])
        ...
        data["ips_stabilized_weight"] = d / data[self.propensity_score_column] * p_treatment + (1 - d) / (
            1 - data[self.propensity_score_column]
        ) * (1 - p_treatment)
        data["tips_stabilized_weight"] = d * p_treatment + (1 - d) * data[self.propensity_score_column] / (
            1 - data[self.propensity_score_column]
        ) * (1 - p_treatment)
        data["cips_stabilized_weight"] = d * (1 - data[self.propensity_score_column]) / data[
            self.propensity_score_column
        ] * p_treatment + (1 - d) * (1 - p_treatment)
