...
class ConfounderDistributionInterpreter(VisualInterpreter):
    ...
    def interpret(self, data: pd.DataFrame):
        ...
        barplot_df_before = df.groupby([self.var_name, treated], observed=True).size().reset_index(name="count")

        # after weights are applied we need to sum over the given weights
        barplot_df_after = df.groupby([self.var_name, treated], observed=True).agg({"weight": np.sum}).reset_index()
