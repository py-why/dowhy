...
def mechanism_change_test(
    ...
    independence_test: Union[
        Callable[[np.ndarray, np.ndarray], float], Callable[[np.ndarray, np.ndarray, np.ndarray], float]
    ] = kernel_based,
) -> float:
...
def distribution_change(
    ...
    mechanism_change_test_func: Callable[
...
def _fit_accounting_for_mechanism_change(
    ...
    old_data: pd.DataFrame,
    new_data: pd.DataFrame,
    mechanism_change_func: Callable[[np.ndarray, np.ndarray, Optional[np.ndarray], Optional[np.ndarray]], float],
    significance_level: float,
    fdr_control_method: Optional[str],
) -> Dict[Any, bool]:
    mechanism_changed_for_node = _check_significant_mechanism_change(
        causal_model_old.graph, old_data, new_data, mechanism_change_func, significance_level, fdr_control_method
    )
    ...
    for node in graph.nodes:
        ...
        all_p_values.append(
            mechanism_change_func(
                old_data[node].to_numpy(), new_data[node].to_numpy(), parents_org_data, parents_new_data
            )
        )
