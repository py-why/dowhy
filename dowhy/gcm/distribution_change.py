...
from dowhy.gcm.independence_test.regression import regression_based
...
def distribution_change(causal_model: ProbabilisticCausalModel,
    ...
    **Research Paper**:
    ...
    # Checking for all nodes whether the mechanism has changed. If the mechanism for a node has not changed, a common
    # mechanism trained on joined data is used for that node in the 'old' and 'new' graph. This is needed to avoid
    # false attributions due to a shift in the support of upstream nodes.
...
def estimate_distribution_change_scores(causal_model: ProbabilisticCausalModel,
    ...
    :param mechanism_change_test_fdr_control_method: The false discovery rate control method for mechanism change
                                                     tests. For more options, checkout `statsmodels manual
                                                     <https://www.statsmodels.org/dev/generated/statsmodels.stats.multitest.multipletests.html>`_.
    :return: A dictionary assigning a score to each node in the causal graph.
    """
    validate_causal_dag(causal_model.graph)

    # Checking for all nodes whether the mechanism has changed. If the mechanism for a node has not changed, the score
    # should be zero. This is needed to avoid false outlier scores due to a shift in the support of upstream nodes.
...
def _estimate_marginal_distribution_change(causal_model_old: ProbabilisticCausalModel,
                                           causal_model_new: ProbabilisticCausalModel,
                                           target_node: Any,
                                           num_samples: int,
                                           difference_estimation_func:
...
def _estimate_distribution_change_score(parent_original_data: np.ndarray,
    ...
    parent_original_data, parent_new_data, target_new_data \
        = shape_into_2d(parent_original_data, parent_new_data, target_new_data)
