...
from dowhy.gcm.util.general import shape_into_2d, set_random_seed
    ...
    **Example usage with dictionary output:**
    ...
    if isinstance(all_results[0], dict):
        ...
        return {key: summary_result[i] for i, key in enumerate(tmp_dict)}, \
               {key: _estimate_percentile_bounds(np.array(tmp_dict[key]).squeeze(), confidence_level)
                for key in tmp_dict}
    else:
        all_results: np.ndarray = shape_into_2d(np.array(all_results))

        return bootstrap_results_summary_func(all_results), \
               np.array([_estimate_percentile_bounds(all_results[:, i], confidence_level) for i in
