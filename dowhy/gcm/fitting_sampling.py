...
def fit_causal_model_of_target(
    ...
    if is_root_node(causal_model.graph, target_node):
        ...
        parent_cols = get_ordered_predecessors(causal_model.graph, target_node)
        causal_model.causal_mechanism(target_node).fit_dataframe(
            X=training_data[parent_cols][~y_nan_mask],
            Y=training_data[target_node][~y_nan_mask],
        )
