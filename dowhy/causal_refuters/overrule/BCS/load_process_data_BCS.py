...
def binarize_features(data, colCateg=[], numThresh=9, negations=False, threshStr=False, **kwargs):
    ...
    quantProb = np.linspace(1.0 / (numThresh + 1.0), numThresh / (numThresh + 1.0), numThresh)
    ...
    for c in data:
        ...
        # Ordinal column
            ...
            if threshStr:
                Anew = pd.DataFrame(
                    Anew, index=data.index, columns=pd.MultiIndex.from_product([[str(c)], ops, thresh[c].astype(str)])
                )
            else:
                Anew = pd.DataFrame(
...
def bin_cont_features(data, colCateg=[], numThresh=9, **kwargs):
    ...
    # Quantile probabilities
    quantProb = np.linspace(0.0, 1.0, numThresh + 2)
    # List of categorical columns
    if type(colCateg) is pd.Series:
        colCateg = colCateg.tolist()
    elif type(colCateg) is not list:
    ...
    for c in data:
        ...
        if (
            (np.issubdtype(data[c].dtype, np.dtype(int).type) or np.issubdtype(data[c].dtype, np.dtype(float).type))
            and (c not in colCateg)
            and valUniq > numThresh + 1
        ):
...
def std_values(data, colCateg=[], **kwargs):
    ...
    for c in A:
        ...
        elif np.issubdtype(A[c].dtype, np.dtype(int).type) or np.issubdtype(A[c].dtype, np.dtype(float).type):
...
def save_train_test(A, y, splits, dirData, fileName, concatMultiIndex=False):
    ...
    if concatMultiIndex and (type(B.columns) is pd.MultiIndex):
        if concatMultiIndex == "BOA":
            ...
            B.columns = (
                B.columns.get_level_values(0).str.replace("_", "-")
                + "_"
                + B.columns.get_level_values(1)
                + B.columns.get_level_values(2)
            )
        else:
            B.columns = B.columns.get_level_values(0) + B.columns.get_level_values(1) + B.columns.get_level_values(2)
        ...
        for (test, (idxTrain, idxTest)) in enumerate(splits):
            ...
            for valid in range(test + 1, numSplit):
                ...
                filePath = os.path.join(
                    dirData, fileName + "_" + format(test, "03d") + "_" + format(valid, "03d") + "_train.csv"
                )
...
def pickle_internal_train(A, y, splits, dirData, fileName):
    ...
    for (test, (idxTrain, idxTest)) in enumerate(splits):
        ...
        for valid in range(test + 1, numSplit):
            ...
            # Save as CSV
            filePath = os.path.join(
                dirData, fileName + "_" + format(test, "03d") + "_" + format(valid, "03d") + "_train.pkl"
            )
...
def example():
    ...
    A, y = load_process_data(
        workDir + fileDir + fileName, rowHeader, colNames, colSep=colSep, col_y=col_y, valEq_y=2, colCateg=colCateg
    )
