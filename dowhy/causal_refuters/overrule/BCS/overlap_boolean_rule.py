...
class OverlapBooleanRule(object):
    ...
    def fit(self, X, y):
        ...
        if not self.silent:
            self.logger.info(
                "Learning Boolean rule set on %s form %s hamming loss"
                % ("CNF" if self.CNF else "DNF", "with" if self.hamming else "without")
            )
        ...
        if self.hamming:
            ...
            v, zNew, Anew = beam_search_no_dup(
                r, X, self.lambda0, self.lambda1, z, K=self.K, eps=self.eps, B=self.B, D=self.D
            )
        ...
        while (v < -self.eps).any() and (self.it < self.iterMax):
            ...
            if self.hamming:
                ...
                v, zNew, Anew = beam_search_no_dup(
                    r, X, self.lambda0, self.lambda1, z, K=self.K, eps=self.eps, D=self.D, B=self.B
                )
    ...
    def greedy_round_(self, X, y, xi=0.5, use_lp=False, gamma=None):
        """
        ...
        if self.CNF:
            while i < MAX_ITER:
                ...
                if A[(y == -1) & (C < 1), :].shape[0] == 0:
                    self.logger.info("Rounded rules cover all reference samples!")
                    break
                ...
                # want
                feasible = (C[(y == 1), np.newaxis] + A[(y == 1), :][:, R]).mean(0) < 1 - self.alpha
                if feasible.sum() == 0:
                    break
