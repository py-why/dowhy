...
class LinearSensitivityAnalysis:
    ...
    def check_r2(self, partialR2_1, partialR2_2):
        ...
        """
        if(any(val >= 1 for val in partialR2_1)):
            self.logger.error('Partial R^2 value should be between 0 and 1')
        if(any(val >= 1 for val in partialR2_2)):
            self.logger.error('Partial R^2 value should be between 0 and 1')
    ...
    def perform_analysis(self):
        ...
        # r2twj_w is partial R^2 of covariate wj with treatment 
        ...
        nd.insert(0,0,1) #inserting bias 
        model = sma.OLS(d, nd)
        ...
        for covariate in self.benchmark_covariates:
            self.r2ywj_tw.append(self.group_partial_r2_func(self.OLSmodel, covariate)) # partial R^2 of covariate Wj with outcome "y", covariates "w" excluding Wj and treatment "t" are partialed out
            self.r2twj_w.append(self.group_partial_r2_func(treatment_results, covariate)) # partial R^2 of covariate Xj with treatment "t", covariates "w" excluding Wj are partialed out
        ...
        for i in range(len(self.benchmark_covariates)):
            ...
            if(any(val >= 1 for val in self.r2tu_w)):
                raise ValueError("Implied bound on r2tu_w >= 1. Try a lower frac_strength_treatment")
            ...
            if(any(val >= 1 for val in r2uwj_wt)):
                raise ValueError("Implied bound on r2uwj_wt >= 1. Try a lower frac_strength_treatment")
        ...
        self.bounds_result = self.compute_bias_adjusted()
    ...
    def robustness_value(self, model = None, covariates = None, t_statistic = None, alpha = 1.0):
        ...
        if(type(t_statistic) in [float, int]):
            t_statistic = pd.Series(t_statistic)

        r_value_q_alpha = 0.5 * (np.sqrt(f_q_alpha ** 4 + (4 * f_q_alpha ** 2)) - f_q_alpha ** 2)
        r_value_q = (cohen_f ** 2 - f_critical ** 2) / (1 + cohen_f ** 2)
        if f_q_alpha < 0:
            r_value = 0
        else:
            r_value = r_value_q_alpha
    ...
    def compute_bias_adjusted(self):
        ...
        bounds_result = pd.DataFrame(data = {
                'r2tu_w': self.r2tu_w,
    ...
    def add_bound(self,bound_value, sensitivity_variable, label_names, x_limit, y_limit):
        ...
        if sensitivity_variable == 'estimate':
            bound_value = self.bounds_result['adjusted_estimate']
        else:
            bound_value = self.bounds_result['adjusted_t']
        ...
        for i in range(len(self.r2tu_w)):
            ...
            #plt.annotate("({:1.3f})".format(bound_value[i]), (self.r2tu_w[i] + x_limit / 10.0, self.r2yu_tw[i] - y_limit / 60.0))
    ...
    def generate_label(self, benchmark_covariate):
        ...
        for i in range(len(self.frac_strength_treatment)):
            ...
            if frac_strength_treatment == frac_strength_outcome:
                signs = str(round(frac_strength_outcome,2))
            else:
                signs = str(round(frac_strength_treatment,2)) + '/' + str(round(frac_strength_outcome,2))
            label = signs + 'x' + ' ' + str(benchmark_covariate)
            label_names.append(label)
        return label_names


    def contour_plot(self,sensitivity_variable, frac_strength_treatment = 1,  estimate_threshold = 0, t_threshold = 2, contour_color = "blue", threshold_color = "red", x_limit = 0.8, y_limit = 0.8):
        """
        ...
        if sensitivity_variable == 'estimate':
            contour_values = [[self.bias_estimate(grid_x[j], grid_y[i], self.estimate) for j in range(len(grid_x))] for i in range(len(grid_y))]
            contour_threshold = estimate_threshold
            unadjusted_estimate = self.estimate
            bound_value = self.bias_estimate(self.r2tu_w, self.r2yu_tw, self.estimate)
        else:
            contour_values = [[self.bias_t(grid_x[j], grid_y[i], estimate_threshold, self.estimate) for j in range(len(grid_x))] for i in range(len(grid_y))]
            contour_threshold = t_threshold
            unadjusted_estimate = (self.estimate - estimate_threshold) / self.standard_error
            bound_value = self.bias_t(self.r2tu_w, self.r2yu_tw, estimate_threshold, self.estimate)
        
        fig, ax = plt.subplots(1, 1, figsize = (7, 7))
        cs = ax.contour(grid_x, grid_y, contour_values, colors = contour_color, linewidths = 0.75, linestyles = "solid")
        
        ax.clabel(cs, inline = 1, fontsize = 8, colors = "blue")
        cs = ax.contour(grid_x, grid_y, contour_values, colors = threshold_color, linewidths = 0.75, linestyles = [(0, (7, 3))], levels = [contour_threshold])
        ax.clabel(cs, inline = 1, fontsize = 8, colors = "blue")

        ax.scatter([0], [0], c = 'k', marker = '^')
        ax.annotate("Unadjusted\n({:1.3f})".format(unadjusted_estimate), (0.0, 0.0))

        plt.xlabel(r"Partial $R^2$ of confounder(s) with the treatment")
        plt.ylabel(r"Partial $R^2$ of confounder(s) with the outcome")
        plt.xlim(-(x_limit / 20.0), x_limit)
        plt.ylim(-(y_limit / 20.0), y_limit)
        ...
        """
        margin_x = 0.1 * x_limit
        margin_y = 0.1 * y_limit
