...
from dowhy.utils.util import generate_moment_function, create_polynomial_function, get_generic_regressor

def get_alpha_estimator(cv, X, max_degree = None, estimator_list = None, estimator_param_list = None, numeric_features = None, plugin_reisz=True):
    """
    ...
    if plugin_reisz:
        ...
        t = X[:, 0]
        propmodel = get_generic_regressor(cv=cv, X=W, Y=t,
                max_degree = max_degree, estimator_list = estimator_list,
                estimator_param_list = estimator_param_list, numeric_features = numeric_features)
        reisz_fn = PluginReisz(propmodel)
    else:
        if estimator_param_list is None:
            estimator_param_list = [{'reisz_functions': [create_polynomial_function(2), create_polynomial_function(1)],
                                    'min_samples_leaf': [10, 50],
