...
def perform_bootstrap_test(estimate, simulations: List):
    ...
    # Reduce estimate.value to a scalar so comparisons work even when the estimator
    # returns an array-valued CATE (e.g. EconML with discrete_outcome=True).
...
def perform_normal_distribution_test(estimate, simulations: List):
    ...
    # Reduce estimate.value to a scalar so the comparison `z_score > 0` works even when
    # the estimator returns an array-valued CATE (e.g. EconML with discrete_outcome=True).
    estimate_value = float(np.mean(estimate.value))
    z_score = (estimate_value - mean_refute_values) / std_dev_refute_values
