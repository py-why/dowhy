...
def identify_mediation(graph: nx.DiGraph, action_nodes: List[str], outcome_nodes: List[str]):
    """Find all valid mediators if they exist.

    Collects every descendant of the treatment nodes that lies on at least one
    directed path to the outcome.  Returning the full set allows downstream
    estimand construction to correctly handle parallel mediators (e.g. both M1
    and M2 in D → M1 → Y, D → M2 → Y).
    """
    mediation_vars: List[str] = []
    ...
    return parse_state(mediation_vars)
