...
class IDIdentifier(CausalIdentifier):
    def __init__(self, graph, estimand_type, method_name="default", proceed_when_unidentifiable=None):
        """
        ...
        try:
            self._tsort_node_names = OrderedSet(
                list(nx.topological_sort(graph._graph))
    ...
    def identify_effect(self, treatment_names=None, outcome_names=None, adjacency_matrix=None, node_names=None):
        ...
        if len(W) != 0:
            return self.identify_effect(
                treatment_names=treatment_names.union(W),
                outcome_names=outcome_names,
                adjacency_matrix=adjacency_matrix,
                node_names=node_names,
            )
        ...
        if len(c_components) > 1:
            ...
            for component in c_components:
                expressions = self.identify_effect(
                    treatment_names=node_names.difference(component),
                    outcome_names=OrderedSet(list(component)),
                    adjacency_matrix=adjacency_matrix,
                    node_names=node_names,
                )
        ...
        for component in c_components_G:
            ...
            if C.is_empty() is None:
                return self.identify_effect(
                    treatment_names=treatment_names.intersection(component),
                    outcome_names=outcome_names,
