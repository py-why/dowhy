...
class TestCausalGraph(object):
    ...
    def test_get_all_nodes(self):
        for flag in [True, False]:
            ...
            res2 = get_all_nodes(self.nx_graph,
                    self.observed_nodes, 
                    include_unobserved_nodes=flag)
            assert set(res1) == set(res2)

    def test_valid_frontdoor_set(self):
        res1 = self.graph_obj.check_valid_frontdoor_set(
                self.action_node, self.outcome_node, ["X0"])
        res2 = check_valid_frontdoor_set(self.nx_graph,
                self.action_node, self.outcome_node, ["X0"])
        assert res1 == res2
    
    def test_valid_mediation_set(self):
        res1 = self.graph_obj.check_valid_mediation_set(
                self.action_node, self.outcome_node, ["X0"])
        res2 = check_valid_mediation_set(self.nx_graph,
                self.action_node, self.outcome_node, ["X0"])
        assert res1 == res2
